namespace EverscaleSdk.Modules.Abi
{
    public class Consts
    {
        public class Commands
        {
            public const string AttachSignature = "abi.attach_signature";
            public const string AttachSignatureToMessageBody = "abi.attach_signature_to_message_body";
            public const string DecodeAccountData = "abi.decode_account_data";
            public const string DecodeMessage = "abi.decode_message";
            public const string DecodeMessageBody = "abi.decode_message_body";
            public const string EncodeMessageBody = "abi.encode_message_body";
            public const string EncodeMessage = "abi.encode_message";
            public const string EncodeAccount = "abi.encode_account";
            public const string EncodeInternalMessage = "abi.encode_internal_message";
            public const string UpdateInitialData = "abi.update_initial_data";
            public const string DecodeInitialData = "abi.decode_initial_data";
            public const string EncodeInitialData = "abi.encode_initial_data";
            public const string DecodeBoc = "abi.decode_boc";
            public const string EncodeBoc = "abi.encode_boc";
        }
    }
}
